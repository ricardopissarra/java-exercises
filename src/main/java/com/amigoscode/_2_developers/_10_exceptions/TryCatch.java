...
public class TryCatch {
    ...
    public static int safeArrayAccess(int[] arr, int index) {
        try {
            ...
            System.out.println("Index out of bounds: %d".formatted(index));
            return -1;
        }
    }
    ...
    public static int safeParseInt(String text) {
        try {
            ...
            System.out.println("Cannot parse: %s".formatted(text));
            return 0;
        }
    }
    ...
    public static int validateAge(int age) {
        if (age < 0) {
            throw new IllegalArgumentException("Age cannot be negative: %d".formatted(age));
        }
        return age;
    }
